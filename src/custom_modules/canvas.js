...
const welcome = (photoUrl = './src/assets/png/ava.png', groupPhoto = './src/assets/png/group.png', pushname = 'Unknown Name', groupName = 'Unknown Group', memberKe = 0) => new Promise((resolve, reject) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        registerFont('./src/assets/font/ChakraPetch-Regular.ttf', { family: 'Chakra Petch' });
        registerFont('./src/assets/font/ChakraPetch-Bold.ttf', { family: 'Chakra Petch', weight: 'bold' });
        ...
        let bgPath = (new Date()).getHours() >= 18 || (new Date()).getHours() <= 5 ? './src/assets/png/bgnight.png' : './src/assets/png/bg.png';
        const bgImg = yield loadImage(bgPath);
        const photoImg = yield loadImage(photoUrl);
        const groupImg = yield loadImage(groupPhoto);
        ...
        ctx.fillStyle = (new Date()).getHours() >= 18 || (new Date()).getHours() <= 5 ? 'white' : 'black';
        ctx.fillText(`Don't forget to introduce`, 430, 340);
        ctx.fillText(`and read the group rules!`, 430, 380);
