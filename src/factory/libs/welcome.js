 ...
 * @returns {string}
 */
const genSVG = ({ wid = '', pp = path.join(src, 'avatar_contact.png'), title = '', name = '', text = '', background = '' } = {}) => __awaiter(void 0, void 0, void 0, function* () {
    let el = {
        code: ['#_1661899539392 > g:nth-child(6) > image', imageSetter, toBase64(yield toImg(barcode(wid.replace(/[^0-9]/g, '')), 'png'), 'image/png')],
 ...
 */
const render = ({ wid = '', pp = toBase64(fs.readFileSync(path.join(src, 'avatar_contact.png')), 'image/png'), name = '', title = '', text = '', background = toBase64(fs.readFileSync(path.join(src, 'Aesthetic', 'Aesthetic_000.jpeg')), 'image/jpeg'), } = {}, format = 'png') => __awaiter(void 0, void 0, void 0, function* () {
    let svg = yield genSVG({
        wid, pp, name, text, background, title
    });
    return yield toImg(svg, format);
});
